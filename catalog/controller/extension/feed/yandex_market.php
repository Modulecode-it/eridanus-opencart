...
class ControllerExtensionFeedYandexMarket extends Controller {
	...
	public function index() {
		if ($this->config->get('feed_yandex_market_status')) {
			...
			(isset($_GET['feed']) == 'goods') ? $export_to = 'goods' : $export_to = 'needyml'; 
			$products = $this->model_export_yandex_market->getProduct($allowed_categories, $out_of_stock_id, $vendor_required, $export_to);
...
//				$data['cbid'] = 15;
				...
				$data['delivery'] = 'true';
				$data['shipment-options'] = $product['shippingtime'];
				$data['outlet'] = $product['quantity'];
//				$data['local_delivery_cost'] = 100;
				$data['name'] = $product['name'];
				$data['vendor'] = $product['manufacturer'];
				$data['vendorCode'] = $product['model'];
				$data['model'] = $product['name'];
				$data['stock_quantity'] = $product['quantity']; //quantity_in_stock 
...
//				$data['barcode'] = $product['sku'];
	...
	private function setOffer($data) {
		...
		if (isset($_GET['feed']) == 'goods') $allowed_tags = array_merge($allowed_tags, array('shipment-options'=>0, 'outlet'=>0));
...
//		}
	...
	private function array2Tag($tags) {
		...
		foreach ($tags as $key => $value) {
			...
			} elseif ($key == 'shipment-options') {
				$retval .= '<' . $key . '><option days="' . $value . '" order-before="15"/></' . $key . '>' . $this->eol;
			} elseif ($key == 'outlet') {
				$retval .= '<' . $key . 's><' . $key . ' id="0" instock="' . $value . '"/></' . $key . 's>'. $this->eol;
			} else {
