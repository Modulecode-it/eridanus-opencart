...
2. На сервере не использовать `git clean -fd` без явного понимания последствий: команда удаляет неотслеживаемые файлы, включая `system/storage/modification/`, если они не закоммичены.
3. После удаления или обновления `system/storage/modification/` нужно заново обновить модификаторы OpenCart в админке и очистить релевантный cache.
4. Если deploy упирается в локальные неотслеживаемые `.agent/`/`agent-deploy-kit/` файлы или права, сначала исправить владельца/права на сервере, а не удалять runtime-файлы OpenCart вслепую.
