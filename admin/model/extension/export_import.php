...
class ModelToolExportImport extends Model {
	...
	protected function storeProductIntoDatabase( &$product, &$languages, &$product_fields, $exist_table_product_tag, $exist_meta_title, &$layout_ids, &$available_store_ids, &$manufacturers, &$weight_class_ids, &$length_class_ids, &$url_alias_ids ) {
		...
		$sql .= "`location`,`stock_status_id`,`model`,`manufacturer_id`,`image`,`shipping`,`shippingtime`,`price`,`points`,`date_added`,`date_modified`,`needyml`,`goods`,`featured`,`date_available`,`weight`,`weight_class_id`,`status`,";
		...
		$sql .= in_array('isbn',$product_fields) ? "'$isbn'," : "";
		$sql .= in_array('mpn',$product_fields) ? "'$mpn'," : "";
		$sql .= "'$location',$stock_status_id,'$model',$manufacturer_id,'$image',$shipping,$shippingtime,$price,$points,";
		$sql .= ($date_added=='NOW()') ? "$date_added," : "'$date_added',";
		$sql .= ($date_modified=='NOW()') ? "$date_modified," : "'$date_modified',";
		$sql .= (strlen($needyml)==1) ? "'$needyml'," : "'1',";
		$sql .= (strlen($goods)==1) ? "'$goods'," : "'1',";
		$sql .= (strlen($featured)==1) ? "'$featured'," : "'1',";
	...
	protected function uploadProducts( &$reader, $incremental, &$available_product_ids=array() ) {
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$needyml = $this->getCell($data,$i,$j++);
			$goods = $this->getCell($data,$i,$j++);
			$featured = $this->getCell($data,$i,$j++);
			...
			$shippingtime = $this->getCell($data,$i,$j++,'0');
	...
	protected function validateProducts( &$reader ) {
		...
		$expected_heading = array
		( "product_id", "name", "needyml", "goods", "featured", "categories", "sku", "upc" );
		if (in_array("ean",$product_fields)) {
		...
		if ($this->use_table_seo_url) {
			$expected_heading = array_merge( $expected_heading, array( "location", "quantity", "model", "manufacturer", "image_name", "shipping", "shippingtime", "price", "points", "date_added", "date_modified", "date_available", "weight", "weight_unit", "length", "width", "height", "length_unit", "status", "tax_class_id", "description") );
		} else {
			$expected_heading = array_merge( $expected_heading, array( "location", "quantity", "model", "manufacturer", "image_name", "shipping", "shippingtime", "price", "points", "date_added", "date_modified", "date_available", "weight", "weight_unit", "length", "width", "height", "length_unit", "status", "tax_class_id", "seo_keyword", "description") );
		}
		if ($exist_meta_title) {
...
//		}
	...
	protected function populateProductsWorksheet( &$worksheet, &$languages, $default_language_id, &$price_format, &$box_format, &$weight_format, &$text_format, $offset=null, $rows=null, &$min_id=null, &$max_id=null) {
		...
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('needyml'),1)+1);
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('goods'),1)+1);
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('featured'),1)+1);
		...
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('shippingtime'),10)+1);
