...
class ControllerExtensionModuleYabuy extends Controller {
	private $tdata = array();
	private $error = array();
	...
	public function index() { 
		...
		if (($this->request->server["REQUEST_METHOD"] == "POST") && $this->validate()) {
			if (strpos($this->request->post['yabuy_yacompany'], '-') > 0) {
				$yacompany_arr = explode('-', $this->request->post['yabuy_yacompany']);
				$yacompany = trim($yacompany_arr[1]);
			}
			else {
				$yacompany = trim($this->request->post['yabuy_yacompany']);
			}
			$this->request->post['yabuy_yacompany'] = $yacompany;
			$this->request->post['yabuy_login'] = trim($this->request->post['yabuy_login']);
			$this->request->post['yabuy_token'] = trim($this->request->post['yabuy_token']);
			$this->model_setting_setting->editSetting("yabuy", $this->request->post);		
			$this->model_setting_setting->editSetting("module_yabuy", array('module_yabuy_status'=>$this->request->post['module_yabuy_status']));
		...
		$this->tdata["breadcrumbs"] = array();
   		...
   		$this->tdata["breadcrumbs"][] = array(
			...
			"href"      => $this->url->link("extension/module/yabuy", "user_token=" . $this->session->data["user_token"], "SSL"),
      		"separator" => " :: "
   		);
		
		$this->tdata["action"] = $this->url->link("extension/module/yabuy", "user_token=" . $this->session->data["user_token"], "SSL");
		
		$this->tdata["cancel"] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=module', "SSL");
		
		if (isset($this->request->post['module_yabuy_status'])) {
			$this->tdata['module_yabuy_status'] = $this->request->post['module_yabuy_status'];
		} else {
			$this->tdata['module_yabuy_status'] = $this->config->get('module_yabuy_status');
		}

		if (isset($this->request->post['yabuy_dbs'])) {
			$this->tdata['yabuy_dbs'] = $this->request->post['yabuy_dbs'];
		} else {
			$this->tdata['yabuy_dbs'] = $this->config->get('yabuy_dbs');
		}
		
		if (isset($this->request->post['yabuy_yacompany'])) {
			$this->tdata['yabuy_yacompany'] = $this->request->post['yabuy_yacompany'];
		} else {
			$this->tdata['yabuy_yacompany'] = $this->config->get('yabuy_yacompany');
		}

		if (isset($this->request->post['yabuy_token'])) {
			$this->tdata['yabuy_token'] = $this->request->post['yabuy_token'];
		} else {
			$this->tdata['yabuy_token'] = $this->config->get('yabuy_token');
		}
		
		if (isset($this->request->post['yabuy_login'])) {
			$this->tdata['yabuy_login'] = $this->request->post['yabuy_login'];
		} else {
			$this->tdata['yabuy_login'] = $this->config->get('yabuy_login');
		}
		
		if (isset($this->request->post['yabuy_app_url'])) {
			$this->tdata['yabuy_app_url'] = $this->request->post['yabuy_app_url'];
		} else {
			$this->tdata['yabuy_app_url'] = $this->config->get('yabuy_app_url');
		}

		if (isset($this->request->post['yabuy_long_id'])) {
			$this->tdata['yabuy_long_id'] = $this->request->post['yabuy_long_id'];
		} else {
			$this->tdata['yabuy_long_id'] = $this->config->get('yabuy_long_id');
		}
		...
		if (isset($this->request->post['yabuy_unpaid_status'])) {
			$this->tdata['yabuy_unpaid_status'] = $this->request->post['yabuy_unpaid_status'];
		} else {
			$this->tdata['yabuy_unpaid_status'] = $this->config->get('yabuy_unpaid_status');
		}
		
		if (isset($this->request->post['yabuy_procesing_status'])) {
			$this->tdata['yabuy_processing_status'] = $this->request->post['yabuy_processing_status'];
		} else {
			$this->tdata['yabuy_processing_status'] = $this->config->get('yabuy_processing_status');
		}
		
		if (isset($this->request->post['yabuy_delivery_status'])) {
			$this->tdata['yabuy_delivery_status'] = $this->request->post['yabuy_delivery_status'];
		} else {
			$this->tdata['yabuy_delivery_status'] = $this->config->get('yabuy_delivery_status');
		}
		
		if (isset($this->request->post['yabuy_pickup_status'])) {
			$this->tdata['yabuy_pickup_status'] = $this->request->post['yabuy_pickup_status'];
		} else {
			$this->tdata['yabuy_pickup_status'] = $this->config->get('yabuy_pickup_status');
		}
		
		if (isset($this->request->post['yabuy_delivered_status'])) {
			$this->tdata['yabuy_delivered_status'] = $this->request->post['yabuy_delivered_status'];
		} else {
			$this->tdata['yabuy_delivered_status'] = $this->config->get('yabuy_delivered_status');
		}
		
		if (isset($this->request->post['yabuy_cancelled_status'])) {
			$this->tdata['yabuy_cancelled_status'] = $this->request->post['yabuy_cancelled_status'];
		} else {
			$this->tdata['yabuy_cancelled_status'] = $this->config->get('yabuy_cancelled_status');
		}
		//--- Соответствие статусов ---
		
		//+++ DBS +++
		if (isset($this->request->post['yabuy_weekend_sat'])) {
			$this->tdata['yabuy_weekend_sat'] = $this->request->post['yabuy_weekend_sat'];
		} else {
			$this->tdata['yabuy_weekend_sat'] = $this->config->get('yabuy_weekend_sat');
		}
		if (isset($this->request->post['yabuy_weekend_sun'])) {
			$this->tdata['yabuy_weekend_sun'] = $this->request->post['yabuy_weekend_sun'];
		} else {
			$this->tdata['yabuy_weekend_sun'] = $this->config->get('yabuy_weekend_sun');
		}
		
        /*
		$this->tdata['modules'] = $this->getShippingModules();
		
		$this->tdata["yabuy_modules"] = array();
		if (isset($this->request->post["yabuy_modules"])) {
			$this->tdata["yabuy_modules"] = $this->request->post["yabuy_modules"];
		} elseif ($this->config->get("yabuy_modules")) { 
			$this->tdata["yabuy_modules"] = $this->config->get("yabuy_modules");
		}
        */
		
		$this->tdata["yabuy_deliveries"] = array();
		if (isset($this->request->post["yabuy_deliveries"])) {
			$this->tdata["yabuy_deliveries"] = $this->request->post["yabuy_deliveries"];
		} elseif ($this->config->get("yabuy_deliveries")) { 
			$this->tdata["yabuy_deliveries"] = $this->config->get("yabuy_deliveries");
		}

		$this->tdata["yabuy_postals"] = array();
		if (isset($this->request->post["yabuy_postals"])) {
			$this->tdata["yabuy_postals"] = $this->request->post["yabuy_postals"];
		} elseif ($this->config->get("yabuy_postals")) { 
			$this->tdata["yabuy_postals"] = $this->config->get("yabuy_postals");
		}
		
		$this->tdata["yabuy_outlets"] = array();
		if (isset($this->request->post["yabuy_outlets"])) {
			$this->tdata["yabuy_outlets"] = $this->request->post["yabuy_outlets"];
		} elseif ($this->config->get("yabuy_outlets")) { 
			$this->tdata["yabuy_outlets"] = $this->config->get("yabuy_outlets");
		}
		...
		if (is_file(DIR_CATALOG . 'controller/yandexbuy/outlets.csv')) {
			...
			$this->tdata["text_csvoutlets"] = sprintf($this->language->get("text_csvoutlets"), 'catalog/controller/yandexbuy/outlets.csv');
		...
		$this->response->setOutput($this->load->view('extension/module/yabuy', $this->tdata));
	}
	...
	public function install() {
		...
		$this->model_extension_event->addEvent('order_status_2_yandexbuy', 'catalog/model/checkout/order/addOrderHistory/after', 'yandexbuy/status/forward');			
	}
	
	public function uninstall() {
		$this->load->model('extension/event');
		$this->model_extension_event->deleteEvent('order_status_2_yandexbuy');
	}
	...
	private function validate() {
		if (!$this->user->hasPermission("modify", "extension/module/yabuy")) {
