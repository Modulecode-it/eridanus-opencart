# Справочник graphify: промпт субагента извлечения (компактный)

Загружай этот файл на Шаге 3, Часть B, когда корпус содержит хотя бы один чанк документа, статьи или изображения. Корпус из чистого кода пропускает Часть B и никогда не читает этот файл. Каждый семантический субагент получает приведённый ниже промпт дословно (подставь FILE_LIST, CHUNK_NUM, TOTAL_CHUNKS и DEEP_MODE).
