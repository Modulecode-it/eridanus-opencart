...
Скопировать содержимое `agent-deploy-kit/core/` → `<project-root>/.agents/`.
...
2. Если подходящий есть — скопировать как `.agents/STACK.md` и при необходимости адаптировать.
...
Создать `.agents/PROJECT_STRUCTURE.md` (шаблон — `agent-deploy-kit/core/templates/project-structure.md`, если есть; иначе с нуля) и заполнить:
...
2. Для каждого: скопировать папку скилла в `.agents/skills/<name>/`, сохранить frontmatter (`name`, `description`) и подпапку `references/`.
3. **Не ломать триггеры**: оставить `name` и `description` как есть.
4. Задокументировать перенос в `.agents/skills/README.md`.

## Шаг 5. Опциональные плагины

- **graphify** (knowledge graph): если нужен — см. `agent-deploy-kit/optional/graphify/README.md`. Копирует skill в `.agents/skills/graphify/`, подключает `hooks.json` (PreToolUse на Bash).
- Другие плагины: добавлять в `.agents/skills/<name>/` по тому же принципу.

## Шаг 6. Корневой `AGENTS.md`

Создать или обновить `<project-root>/AGENTS.md` — компактную точку входа для любого агента. Содержание:
- ссылка на `.agents/README.md` и порядок чтения;
...
1. Проверить, что `.agents/INDEX.md` пустой и готов к работе.
2. Проверить `.agents/CHANGELOG.md` — добавить запись о развёртывании.
3. Закоммитить `.agents/` + `AGENTS.md` отдельным коммитом: `chore(agent): deploy agent memory system`.
...
- [ ] Существующие скиллы перенесены в `.agents/skills/`.
