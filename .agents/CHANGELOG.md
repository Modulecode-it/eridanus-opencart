...
### Added
- Сводка «Вопросы к заказчику» в `specs/backlog.md` (собрана из отчётов задач 1.2–1.9): наложка Яндекса, реальные ФИО/email отправителя, судьба MeaSoft/flat и данные новой ТК, активация тарифов 5Пост, тест-отправление 40274852, ключ Яндекс.Карт, вес товаров, приёмка dev/перенос на боевой, `simple_twig_fix`; напоминалка про серверный cron.
- В `specs/backlog.md` добавлена «Версия для отправки заказчику (простыми словами)» — готовое письмо без технических терминов (технические пункты намеренно исключены).
